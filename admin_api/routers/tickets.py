...
@auth_required
def index():
    """РџРѕР»СѓС‡РµРЅРёРµ РІСЃРµС… С‚РёРєРµС‚РѕРІ"""
...
def ticket_page(id: int):
    """РџРѕР»СѓС‡РµРЅРёРµ С‚РёРєРµС‚Р° РїРѕ ID"""
    ticket = basic_get(Ticket, id=id)
    if not ticket:
        return jsonify({"error": "РўРёРєРµС‚ РЅРµ РЅР°Р№РґРµРЅ"}), 404
...
def post_ticket_message(id):
    """Р”РѕР±Р°РІР»РµРЅРёРµ СЃРѕРѕР±С‰РµРЅРёСЏ РІ С‚РёРєРµС‚"""
    ticket = basic_get(Ticket, id=id)
    if not ticket:
        return jsonify({"error": "РўРёРєРµС‚ РЅРµ РЅР°Р№РґРµРЅ"}), 404
    ...
    return jsonify({"message": "РЎРѕРѕР±С‰РµРЅРёРµ РґРѕР±Р°РІР»РµРЅРѕ", "ticket_id": ticket.id})

@tickets_router.get('/tickets/status')
@auth_required
def status_sorted_tickets(status: bool):
    """Получение всех тикетов с определенным статусом"""
...
def date_sorted_tickets(date: datetime):
    """Получение всех тикетов по определенной дате"""
